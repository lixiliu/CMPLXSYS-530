...
HW 3: Langton's Ants
@author: lixi
Model 5 - single ant, fixed boundary
"""
# setup
import matplotlib
...
width = 10          # uses x, always config[y,x], index starts from 0 # 50
height = 10         # uses y, always config[y,x] 50, index starts from 0 # 50
...
def step():
    ...
    for ant in A:
        ...
        if abs(orientation+360)%360 == 0:     # filter for North
            orientation = 0
            if ant_y < height-1:        
                ant_y = (ant_y+1)       # step up 1
            else:
                ant_y = (ant_y-1)       # fixed boundary, turn around & step 1
        elif abs(orientation+360)%360 == 90:  # filter for East
            orientation = 90
            if ant_x < width-1:
                ant_x = (ant_x+1)       # step right 1
            else:
                ant_x = (ant_x-1)       # fixed boundary, turn around & step 1
        elif abs(orientation+360)%360 == 180: # filter for South
            orientation = 180
            if ant_y < height-1:
                ant_y = (ant_y-1)      # step down 1
            else:
                ant_y = (ant_y-1)       # fixed boundary, turn around & step 1
        else:                             # filter for West
            orientation = 270
            if ant_x < width-1:
                ant_x = (ant_x-1)%width # step left 1
            else:
                ant_x = (ant_x-1)       # fixed boundary, turn around & step 1
